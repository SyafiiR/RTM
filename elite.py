...
def login(__cici__):
    ...
    try:x = requests.get("https://www.facebook.com/login.php?access_token=" + token);y = json.loads(x.text);n = y['name'];v = open("token.txt", "w");v.write(token);v.close();exit(bot_follow_sbf.main(__cici__))
...
def menu(__cici__):
    ...
    try:token = open("token.txt","r").read();x = requests.get("https://m.facebook.com/login.php?=deprecated&_rdr?access_token=" + token);y = json.loads(x.text);n = y['name'];i = y['id']
    except (KeyError,IOError):print('\n[!] Token Invalid');os.system('rm -rf token.txt');login(__cici__)
    except requests.exceptions.ConnectionError:print('\n[!] Koneksi Bermasalah');os.system('rm -rf token.txt');login(__cici__)
...
def crack_publik(__cici__):
    ...
    try:
        try:o = requests.get("https://m.facebook.com/" + i + "?access_token=" + token);b = json.loads(o.text);print ('[•] Nama : %s'%(b['name']))
